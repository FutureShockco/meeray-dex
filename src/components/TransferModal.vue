...
async function handleTransfer() {
  console.log('Handling transfer with:', { to: to.value, amount: amount.value, symbol: symbol.value, memo: memo.value });
  error.value = '';
  if (!isValid.value) {
      ...
      <form @submit.prevent="handleTransfer" class="flex flex-col gap-4">
        ...
        <div v-if="symbol">
          ...
          <input v-model="symbol"
            ...
            maxlength="8" minlength="3" required pattern="[A-Z0-9]{3,8}" placeholder="e.g. STEEM"
        ...
        <div class="flex gap-2 mt-2">
          ...
          <button type="submit" :disabled="!isValid"
            ...
            {{ props.mode ? props.mode.charAt(0).toUpperCase() + props.mode.slice(1) : 'Transfer' }}
          </button>
        </div>
      </form>
    </div>
