    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div
          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="relative">
                ...
                <button @click="setMaxAmount" type="button"
            ...
            <div class="text-xs text-gray-500 dark:text-gray-400 mt-2 p-2 bg-gray-100 dark:bg-gray-800 rounded">
              ...
              <div class="text-red-500 mt-2 p-2 bg-red-100 dark:bg-red-900 rounded">
                ...
                <div>Raw Amount (shifted by precision): {{ amount ? new BigNumber(amount).shiftedBy(18).toString() : '0'
                  }}</div>
                <div>Min Check: {{ farm?.rawMinStakeAmount === '0' || (amount ? new
                  BigNumber(amount).shiftedBy(18).isGreaterThanOrEqualTo(new BigNumber(farm?.rawMinStakeAmount || '0'))
                  : false) }}</div>
                <div>Max Check: {{ farm?.rawMaxStakeAmount === '0' || (amount ? new
                  BigNumber(amount).shiftedBy(18).isLessThanOrEqualTo(new BigNumber(farm?.rawMaxStakeAmount || '0')) :
                  false) }}</div>
                <div>Balance Check: {{ amount ? new BigNumber(amount).shiftedBy(18).isLessThanOrEqualTo(new
                  BigNumber(userBalance || '0')) : false }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-50 dark:bg-gray-800 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          <AppButton @click="handleStake" :disabled="!canStake || loading" variant="primary" size="lg"
            class="w-full sm:w-auto sm:ml-3">
            <span v-if="loading">Staking...</span>
            <span v-else>Stake Tokens</span>
          </AppButton>

          <AppButton @click="closeModal" variant="secondary" size="lg" class="w-full sm:w-auto mt-3 sm:mt-0">
            Cancel
          </AppButton>
        </div>
      </div>
