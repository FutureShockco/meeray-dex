        ...
        <div>
          ...
          <div v-if="lpPositions.length === 0"
            ...
            <div class="flex-1">
              ...
              <div class="text-xs text-gray-500 dark:text-gray-400">Login with your Steem account to view your current positions.
              </div>
            </div>
          </div>
          <div v-else
            ...
            <table class="min-w-full text-sm">
              <thead class="text-gray-500 dark:text-gray-400">
                <tr>
                  ...
                  <th class="px-4 py-2 font-semibold">Fee</th>
                ...
                <tr v-else v-for="(pool, i) in pools" :key="pool.id || i"
                  ...
                  <td class="px-4 py-2">
                    <router-link :to="{ path: '/pool', query: { poolId: pool.id } }"
                      class="block w-full h-full text-inherit no-underline">{{ i + 1 }}</router-link>
                  </td>
                  ...
                  <td class="px-4 py-2 text-gray-900 dark:text-white">{{ pool.feeTier / 1000 + '%' || '0.3%' }}</td>
                  <td class="px-4 py-2 text-gray-900 dark:text-white">{{ getTvlUsd(pool) }}</td>
                  <td class="px-4 py-2 text-gray-900 dark:text-white">{{ pool.aprA !== undefined ? (pool.aprA * 100).toFixed(2) + '%' : '--' }}</td>
                  <td class="px-4 py-2 text-gray-900 dark:text-white">{{ pool.aprB !== undefined ? (pool.aprB * 100).toFixed(2) + '%' : '--' }}</td>
                  <td class="px-4 py-2 text-gray-900 dark:text-white">
                    <span v-if="pool.aprA !== undefined && pool.aprB !== undefined">
                      {{ ((pool.aprA + pool.aprB) * 100).toFixed(2) }}%
                    </span>
                    <span v-else>--</span>
                  </td>
                  <td class="px-4 py-2 text-gray-900 dark:text-white">
                    <span v-if="pool.fees24hA !== undefined">{{ Number(pool.fees24hA).toLocaleString(undefined, { maximumFractionDigits: 4 }) }}</span>
                    <span v-else>--</span>
                  </td>
                  <td class="px-4 py-2 text-gray-900 dark:text-white">
                  ...
                  <td class="px-4 py-2 text-gray-900 dark:text-white">
                    ...
                    <router-link :to="{ path: '/swap', query: { tokenIn: pool.tokenA_symbol, tokenOut: pool.tokenB_symbol } }"
          ...
          <div v-for="pool in topPools" :key="pool.id"
            ...
            <div>
              ...
              <div class="text-xs text-gray-500 dark:text-gray-400 flex gap-2">
                ...
                <span v-if="pool.aprA !== undefined && pool.aprB !== undefined">Combined: {{ ((pool.aprA + pool.aprB) * 100).toFixed(2) }}%</span>
              </div>
            </div>
            <div class="text-right">
              <div class="font-semibold text-primary-400">{{ pool.aprA !== undefined && pool.aprB !== undefined ? ((pool.aprA + pool.aprB) * 100).toFixed(2) + '%' : '--' }}</div>
            </div>
          </div>
